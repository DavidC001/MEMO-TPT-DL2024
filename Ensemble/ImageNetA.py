...
def TPT(device, naug=30):
    ...
    imageNetA, _, imageNetACustomNames, imageNetAMap, _, _, _, _ = get_datasets(datasetRoot, naug, all_classes=False)
    tpt = TPT_get_model(device=device, classnames=imageNetACustomNames)
    
    return tpt, imageNetA, imageNetAMap
...
def memo(device):
    ...
    imageNet_A, _ = get_dataloaders('datasets', memo_transforms)

    return memo, imageNet_A
