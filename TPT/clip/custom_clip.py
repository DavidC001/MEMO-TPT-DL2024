...
class PromptLearner(nn.Module):
    ...
    def reset_classnames(self, classnames, arch):
        """
        Parses the classnames, tokenizes them and stores the embedded version
        of the prefix and suffix.
        By default the length of the tokenized prompts is 77, the extra spaces are zero-padded.


        Args:
            classnames (list): A list of class names.
            arch (str): The architecture to use for loading the CLIP model.
        ...
        # this will be used when generating the context vector
        name_lens = [len(_tokenizer.encode(name)) for name in classnames]

        # concatenate prefix (e.g., "a photo a ") and class name (e.g., "zebra")
